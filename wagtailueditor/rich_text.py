...
class UEditorRichTextArea(WidgetWithScript, widgets.HiddenInput):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for feature in self.features:
            plugin = feature_registry.get_editor_plugin('ueditor', feature)
            if plugin:
    ...
    def render(self, name, value, attrs=None):
        ...
        attrs['data-ueditor-input'] = True

        translated_value = self.translate_value(value)
        return super().render(name, translated_value, attrs)

    def render_js_init(self, id_, name, value):
        #return "window.UE.getEditor('container')"
        return "window.UE.getEditor({0});".format(json.dumps(id_))
        # return "window.ueditor.initEditor('#{id}', {opts}, document.currentScript)".format(
